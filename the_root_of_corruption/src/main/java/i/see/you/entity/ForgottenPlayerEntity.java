...
import i.see.you.procedures.TrueplayerProcedure;
import i.see.you.procedures.ForgottenPlayerTickupdateProcedure;
import i.see.you.procedures.CanForgottenProcedure;
import i.see.you.procedures.AttackNotTargetProcedure;
...
public class ForgottenPlayerEntity extends Monster {
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(3, new MeleeAttackGoal(this, 1.2, true) {
			@Override
			protected boolean canPerformAttack(LivingEntity entity) {
				return this.isTimeToAttack() && this.mob.distanceToSqr(entity) < 256 && this.mob.getSensing().hasLineOfSight(entity);
			}
		});
		this.goalSelector.addGoal(4, new LeapAtTargetGoal(this, (float) 0.5));
		this.goalSelector.addGoal(5, new AvoidEntityGoal<>(this, Monster.class, (float) 6, 1, 1.2));
	...
	protected void dropCustomDeathLoot(ServerLevel serverLevel, DamageSource source, boolean recentlyHitIn) {
		...
		this.spawnAtLocation(new ItemStack(TheRootOfCorruptionModItems.HOPE.get()));
	}
	...
	public void thunderHit(ServerLevel serverWorld, LightningBolt lightningBolt) {
		...
		TrueplayerProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}

	@Override
	public boolean causeFallDamage(float l, float d, DamageSource source) {
		CanForgottenProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ());
	...
	public boolean hurt(DamageSource damagesource, float amount) {
		...
		AttackNotTargetProcedure.execute(world, x, y, z, entity, sourceentity);
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor world, DifficultyInstance difficulty, MobSpawnType reason, @Nullable SpawnGroupData livingdata) {
		...
		TrueplayerProcedure.execute(world, this.getX(), this.getY(), this.getZ(), this);
		return retval;
	}
	...
	public void baseTick() {
		super.baseTick();
		ForgottenPlayerTickupdateProcedure.execute(this.level(), this.getX(), this.getY(), this.getZ(), this);
	}

	public static void init(RegisterSpawnPlacementsEvent event) {
		event.register(TheRootOfCorruptionModEntities.FORGOTTEN_PLAYER.get(), SpawnPlacementTypes.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return CanForgottenProcedure.execute(world, x, y, z);
