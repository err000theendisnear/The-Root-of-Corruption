...
public class GiveAllBuffToEntityProcedure {
	public static void execute(Entity entity, boolean beacon, boolean particle, double level, double time) {
		...
		if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide()) {
			...
			for (MobEffect effect : BuiltInRegistries.MOB_EFFECT) {
				if (BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effect).value().getCategory() == net.minecraft.world.effect.MobEffectCategory.BENEFICIAL && BuiltInRegistries.MOB_EFFECT.wrapAsHolder(effect) != TheRootOfCorruptionModMobEffects.CORRUPTION)
