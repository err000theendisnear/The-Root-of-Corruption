		...
		if (!(player == null)) {
			...
			if (LookentityProcedure.execute(player, entity)) {
				...
				if (Math.random() < 0.5) {
					...
					BombProcedure.execute(world, player.getX(), player.getY(), player.getZ());
					TheRootOfCorruptionMod.queueServerWork(20, () -> {
						KillentityProcedure.execute(world, (Entity) world.getEntitiesOfClass(Player.class, AABB.ofSize(new Vec3(x, y, z), 666, 666, 666), e -> true).stream().sorted(new Object() {
							Comparator<Entity> compareDistOf(double _x, double _y, double _z) {
								return Comparator.comparingDouble(_entcnd -> _entcnd.distanceToSqr(_x, _y, _z));
							}
						}.compareDistOf(x, y, z)).findFirst().orElse(null));
