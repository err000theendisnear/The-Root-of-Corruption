...
public class SaveusLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<SaveusLeaveDecorator> CODEC = MapCodec.unit(SaveusLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("the_root_of_corruption:saveus_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public SaveusLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, TheRootOfCorruptionModBlocks.MISSINGNO.get().defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(TheRootOfCorruptionModBlocks.MISSINGNO.get().defaultBlockState(), direction));
