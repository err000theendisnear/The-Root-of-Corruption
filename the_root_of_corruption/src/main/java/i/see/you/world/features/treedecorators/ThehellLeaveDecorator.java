...
public class ThehellLeaveDecorator extends LeaveVineDecorator {
	public static MapCodec<ThehellLeaveDecorator> CODEC = MapCodec.unit(ThehellLeaveDecorator::new);
	public static TreeDecoratorType<?> DECORATOR_TYPE = new TreeDecoratorType<>(CODEC);

	@SubscribeEvent
	public static void registerTreeDecorator(RegisterEvent event) {
		event.register(Registries.TREE_DECORATOR_TYPE, ResourceLocation.parse("the_root_of_corruption:thehell_tree_leave_decorator"), () -> DECORATOR_TYPE);
	}

	public ThehellLeaveDecorator() {
		super(0.25f);
	}
	...
	private static void addVine(BlockPos pos, Direction direction, TreeDecorator.Context context) {
		context.setBlock(pos, Blocks.NETHERRACK.defaultBlockState());
		int i = 4;
		for (BlockPos blockpos = pos.below(); context.isAir(blockpos) && i > 0; --i) {
			context.setBlock(blockpos, oriented(Blocks.NETHERRACK.defaultBlockState(), direction));
